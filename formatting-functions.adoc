...
....
    ...
    == "390625KB"
    >> format-bytes/as 400000000 'GB            ; Note rounding!
    == "0GB"
    >> format-bytes/as/to 400000000 'GB .01
    == "0.37GB"
...
support custom formats. Useful in code generators.
...
....
    ...
    == "Y"
    >> format-logic true [Yeah! No-way!]
    == "Yeah!"
    >> format-logic false [Yeah! No-way!]
    == "No-way!"
    >> mold-logic true 'on-off
    == on-off
    >> mold-logic true 'yes-no
    == yes-no
....
...
less in others. By default, it works just like `form`, but give you the ability
...
....
    ...
    eng     Engineering form; 1-3 digits left of decimal, with an exponent
            that is always a multiple of 3.
...
at 8 places, you could use this function:
	...
	cust-exp-fn: formatting/make-custom-exp-fn [
	    either any [n < -7  n > 7][n][none]
	]
...
TBD: [
    Should we truncate based on mask precision? Currently it is up
...
This concept is taken from the world of spreadsheets. Rather than you manually
select a mask by looking at a number's value, you give the system options for
each case you want to handle. Normally `<POSITIVE>;<NEGATIVE>;<ZERO>;<TEXT>`.
In my R2 system, I also allowed blocks with named selectors, which I plan to 
extend in the Red system to support functions as tests, which will let the
system handle special values like `1.#INF` and `1.#NaN`. Another likely 
extension is the ability to use short-format specifications and named styles
for each section or test.
...
=== styles
    ...
    ;TBD accounting      [pos " #,##0.00 " neg "(#,##0.00)" zero "-" none ""]
...
no key is given. Thats where your field starts with `:` rather than `/`.
...
That is, 0 or more flags, followed by an optional width and precision, with a
...
....
    ...
    0       Set fill char to 0 instead of space. (Can't be first flag char)
    <       Left align
    >       Right align (default)
    Zz      Set fill char to 0 instead of space. (May remove 0 flag. This is better.)
    º       Ordinal (just an idea)
    $¤      Money
...
apply-test
    INPUT:  ":/pi"
    VALUE:  123.456
    OUTPUT: "123.4563.141592653589793"
apply-test
...
apply-test
    ...
    OUTPUT: {Color: 8080      , number1/    , http://  , float:      }
apply-test
...
HOWEVER, an option would be to use tag! values and apply exactl the same 
...
apply-test
    INPUT:  [Color: (:< 10) number1 (/3) number2 () xxx]
    VALUE:  ["Red" 2 3 -45.6]
    OUTPUT: "Color: Red        number1 3 number2 Red xxx"
apply-test
